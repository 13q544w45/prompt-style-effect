...
def evaluate_predictions(truth_path, prediction_path, output_path):
    ...
    for i in range(total):
        gt_answer = normalize(gt_data[i]["answer"])
        gpt_answer = normalize(pred_data[i]["gpt3_answer"])
        is_correct = gt_answer == gpt_answer
        if is_correct:
            correct += 1
        results.append({
            "id": i,
            "question": gt_data[i].get("question", "N/A"),
            "truth": gt_answer,
...
def main():
    truth_path = "data/sample_dataset/qa_100.jsonl"
    prediction_path = "data/model_outputs/closedbookqa_instruction_gpt3.jsonl"
    output_path = "evaluation/closedbookqa_instruction.jsonl"
